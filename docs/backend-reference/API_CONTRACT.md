...
#### `GET /api/vitrine/actualites/{id}` — Public, par identifiant
**Accès** : Public

Retourne l'actualité si elle existe **et** que `publie = true`.
Si elle est inexistante **ou** dépubliée (`publie = false`) : **404 dans les deux cas** — le frontend ne peut pas distinguer les deux situations (sécurité de surface d'exposition).

**Réponse 200** : `ActualiteResponse` (mêmes champs que le listing)  
**Erreurs** : 404 (inexistant ou dépublié)

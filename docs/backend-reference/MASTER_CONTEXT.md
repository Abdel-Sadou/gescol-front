...
## 10. Configuration de démarrage — pièges déjà résolus (PROMPT_18)

- `GescolBackendApplication` (package `com.gescol.backend`) porte
  explicitement `@EnableJpaRepositories(basePackages = "com.gescol")` et
  `@EntityScan(basePackages = "com.gescol")` — sans ça, `@SpringBootApplication`
  seul ne détecte aucun repository JPA en démarrage réel (`bootRun`), même si
  les tests `@SpringBootTest` semblent fonctionner. Ne jamais retirer ces
  deux annotations.
- `Eleve.sexe` porte `@Column(nullable = false, columnDefinition = "VARCHAR(1)")`
  — pas de `length = 1` seul sur un enum `@Enumerated(EnumType.STRING)`, ça
  force Hibernate 6 à valider contre `CHAR(1)` alors que la colonne réelle
  (migration V7) est `VARCHAR(1)`. Pattern à vérifier si un problème similaire
  apparaît sur un autre enum mappé en colonne courte.
